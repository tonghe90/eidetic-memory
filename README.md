...
自动采集浏览记录、邮件、AI 对话历史，通过 LLM 增量编译成结构化知识库，支持本地搜索与溯源链接。
...
```
Chrome Extension (eidetic-memory-sync)
...
| 页面        | 功能                                         |
| --------- | ------------------------------------------ |
| Dashboard | 待摄取数量（按来源分类）、摄取历史记录、「开始摄取」按钮、实时进度条         |
...
| 凭证存储    | 本地文件（`data/*.json`，已在 `.gitignore` 中排除）              |
...
2. 用户类型选 **外部**，填写应用名称（如 `Sheldon's Eidetic Memory`）
...
- OAuth token 仅存本地文件（`data/*.json`），已在 `.gitignore` 中排除，不会入库
